...
// Vercel mounts function at /api — strip prefix for route matching
...
// ============ STAGE MODE ============
interface StageTeam {
  id: number; name: string; color: string; emblem: string; code: string;
  score: number; correct: number; total: number; streak: number;
  tacticalLoadout: string[];
}

interface StageMatchState {
  id: number; roomCode: string; hostId: number; teams: StageTeam[];
  questions: any[]; currentQuestionIndex: number;
  phase: "lobby" | "intro" | "question" | "buzzed" | "answered" | "rebuzz" | "ended";
  buzzerTeamId: number | null; wrongAttempts: number;
  timerSeconds: number; timerStartedAt: number | null;
  originalTimerSeconds: number; buzzedOptionId: number | null;
  domains: string[]; difficulty: string; totalQuestions: number;
}
...
const DOMAIN_CATEGORIES: Record<string, string[]> = {
  cyber_systems: ["technology", "security"], cognitive_analysis: ["logic", "intelligence"],
  historical_archives: ["history"], threat_intelligence: ["security", "intelligence"],
  scientific_division: ["technology"], behavioral_analysis: ["intelligence"],
  global_mapping: ["history", "technology"], cipher_division: ["security", "intelligence"],
};
const DIFFICULTY_CONFIG: Record<string, { diffRange: number[] }> = {
  recruit: { diffRange: [1, 3] }, agent: { diffRange: [3, 6] },
  elite: { diffRange: [5, 8] }, omega: { diffRange: [7, 10] },
};

// POST /stage/create
apiRouter.post("/stage/create", async (req, res) => {
  ...
  for (let i = 0; i < (teamCount || 2); i++) {
    ...
    usedCodes.add(code);
    teams.push({ id: i + 1, name: "", color: TEAM_COLORS[i % 8], emblem: TEAM_EMBLEMS[i % 8], code, score: 0, correct: 0, total: 0, streak: 0, tacticalLoadout: [] });
  }
  stageMatches.set(matchId, {
    id: matchId, roomCode, hostId: user.id, teams, questions: [], currentQuestionIndex: 0,
    phase: "lobby", buzzerTeamId: null, wrongAttempts: 0,
    timerSeconds: timerSeconds || 30, timerStartedAt: null,
    originalTimerSeconds: timerSeconds || 30, buzzedOptionId: null,
    domains: domains || [], difficulty: difficulty || "agent", totalQuestions: questionCount || 10,
  });
  res.json({ matchId, roomCode, teams: teams.map(t => ({ id: t.id, name: t.name, color: t.color, emblem: t.emblem, code: t.code })) });
});

// POST /stage/batch-config
apiRouter.post("/stage/batch-config", async (req, res) => {
  const user = await getUserFromToken(req.headers.authorization);
  if (!user) { res.status(401).json({ error: "Not authenticated" }); return; }
...
// POST /stage/buzzer-connect
apiRouter.post("/stage/buzzer-connect", async (req, res) => {
  const { teamCode } = req.body;
  if (!teamCode) { res.status(400).json({ error: "Team code required" }); return; }
  const match = Array.from(stageMatches.values()).find(m => m.teams.some(t => t.code === teamCode.toUpperCase()));
...
// POST /stage/start
apiRouter.post("/stage/start", async (req, res) => {
  const user = await getUserFromToken(req.headers.authorization);
  if (!user) { res.status(401).json({ error: "Not authenticated" }); return; }
  ...
  const fullQs = await Promise.all(questions.map(async (q) => {
    ...
    return {
      id: q.id, questionText: q.questionText, difficulty: q.difficulty, category: q.category,
      options: options.map(o => ({ id: o.id, text: o.text })),
      timeLimit: q.timeLimitSeconds || match.timerSeconds, type: q.type,
  ...
  if (fullQs.length > 0) {
    ...
    match.timerSeconds = fullQs[0].timeLimit || match.originalTimerSeconds;
    match.timerStartedAt = Date.now();
  }
...
// POST /stage/buzz
apiRouter.post("/stage/buzz", async (req, res) => {
...
// POST /stage/answer
apiRouter.post("/stage/answer", async (req, res) => {
  const user = await getUserFromToken(req.headers.authorization);
  if (!user) { res.status(401).json({ error: "Not authenticated" }); return; }
  ...
  if (match.wrongAttempts === 0) {
    ...
    res.json({ success: true, correct: false, pointsGained: 0, rebuzz: true, rebuzzTime, rebuzzTeamName: team.name });
    return;
  }
...
// POST /stage/next
apiRouter.post("/stage/next", async (req, res) => {
  const user = await getUserFromToken(req.headers.authorization);
  if (!user) { res.status(401).json({ error: "Not authenticated" }); return; }
  const { matchId } = req.body;
  const match = stageMatches.get(matchId);
  ...
  const q = match.questions[match.currentQuestionIndex];
  match.phase = "question";
  match.timerSeconds = q.timeLimit || match.originalTimerSeconds;
  match.timerStartedAt = Date.now();
...
// GET /stage/:id
apiRouter.get("/stage/:id", async (req, res) => {
  ...
  res.json({
    id: match.id, roomCode: match.roomCode, phase: match.phase,
    ...
    question: qStrip,
    domains: match.domains, difficulty: match.difficulty,
  });
});

// POST /stage/timeout — client notifies that time expired
apiRouter.post("/stage/timeout", async (req, res) => {
  const { matchId } = req.body;
  const match = stageMatches.get(matchId);
  if (!match) { res.status(404).json({ error: "Match not found" }); return; }
